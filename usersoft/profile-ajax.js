...
function CheckOldPass(cust_edit_id,cust_edit_old_pass){
	...
	var url = "profile-ajax.php?mode=checkPass&cust_id="+cust_id+"&cust_old_pass="+cust_old_pass;
...
function ChangePasswordCustomer(event){
	...
	if(cust_pass_new!=cust_pass_new_confirm){
		...
		var url = "profile-ajax.php?mode=CustChangePassword&cust_id="+cust_id+"&cust_pass_new="+cust_pass_new; // the script where you handle the form input.
